...
import 'package:rada/model/detailsmodel.dart';
import 'package:rada/model/hivmodel.dart';
import 'package:rada/model/messagesmodel.dart';
...
class DetailsHelper {
  ...
  Future<List<DetailsModel>> getMessages() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.rawQuery("SELECT * FROM $TABLE ORDER BY $id ASC");
    List<DetailsModel> recordings = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        recordings.add(DetailsModel.fromMap(maps[i]));
      }
    }
    return recordings;
  }
  ...
  Future<int> update(DetailsModel messages) async {
    var dbClient = await db;
